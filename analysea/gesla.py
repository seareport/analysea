...
class GeslaDataset:
    ...
    def __init__(self, meta_file: str, data_path: str):
        ...
        self.meta.loc[:, "end_date_time"] = [pd.to_datetime(d) for d in self.meta.loc[:, "end_date_time"]]
    ...
    def file_to_pandas(self, filename, return_meta=True):
        ...
        with open(self.data_path + filename, "r") as f:
            data = pd.read_csv(
                f,
                skiprows=41,
                names=["date", "time", "slevel", "qc_flag", "use_flag"],
                sep="\s+",
                parse_dates=[[0, 1]],
                index_col=0,
            )
